...
public class PointCloudRenderer {
    ...
    public void draw(float[] cameraView, float[] cameraPerspective) {
        ...
        if (numPoints > 0) {
            ...
            System.out.println("size of buffer " + lastPointCloud.getPoints().limit());
            FloatBuffer fb = lastPointCloud.getPoints();
            while (fb.hasRemaining()) {
                float x = fb.get();
                float y = fb.get();
                float z = fb.get();
                float confidence = fb.get();
                FeaturePoint fp = new FeaturePoint(x, y, z, confidence);
                System.out.println(fp.toString());
                Pair<Float, Float> screenXY = getScreenCoordinates(x, y, z, modelViewProjection);
                System.out.println("x: " + screenXY.first + " , " + screenXY.second);
            }
        }
    ...
    public Pair<Float, Float> getScreenCoordinates(float x, float y, float z, float[] modelViewProjection) {
        float[] input3dPoint = new float[4];
        input3dPoint[0] = x;
        input3dPoint[1] = y;
        input3dPoint[2] = z;
        input3dPoint[3] = 1;
        float[] result = new float[4];
        Matrix.multiplyMV(result, 0, modelViewProjection, 0, input3dPoint, 0);
    ...
    class FeaturePoint {
        ...
        public FeaturePoint(float x, float y, float z, float confidence) {
            this.x = x;
            this.y = y;
            this.z = z;
